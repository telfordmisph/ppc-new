		...
		return (
			<div className="join">
				<button
					...
					disabled={current <= 1}
					onClick={() =>
						router.visit(meta.links.find((l) => l.label === "&laquo;")?.url)
					}
				...
				<button
					...
					onClick={() =>
						router.visit(meta.links.find((l) => l.label === "&raquo;")?.url)
					}
			...
			>
				...
				{dateRangeSearch ? (
					...
					<div className="flex flex-col w-full gap-2 sm:flex-row sm:items-center sm:w-auto">
						...
						/>
						<button type="submit" className="px-2 btn btn-sm btn-primary">
							<svg
