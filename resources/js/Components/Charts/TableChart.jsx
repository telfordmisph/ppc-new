	...
	{table.getHeaderGroups().map((headerGroup) => (
		...
		{headerGroup.headers.map((header) => {
			...
			return (
				...
				className={`bg-base-200 whitespace-nowrap font-light relative
                                            ${isPin ? "min-w-14 text-left font-semibold" : "text-right"}`}
					>
					{flexRender(
				...
				return (
					...
					{row.getVisibleCells().map((cell) => {
						...
						return (
							...
							key={cell.id}
							style={{ width: cell.column.getSize() }}
							className={`${zebra} text-right whitespace-nowrap
                                                ${isNumeric && isNumericValue(val) ? "font-mono" : ""}`}
							>
							{flexRender(
								cell.column.columnDef.cell,
								cell.getContext(),
