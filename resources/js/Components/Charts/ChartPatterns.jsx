import React, { memo } from "react";

export const PATTERN_SCALE = "scale(0.3)";

export const PatternDef = memo(function PatternDef({ id, pattern, fill }) {
	console.log("🚀 ~ PatternDef ~ id:", id, pattern, fill);
	if (!id || !pattern || !fill) return null;

	switch (pattern) {
		case "dashes":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="40"
					height="1"
					patternTransform={PATTERN_SCALE}
				>
					<path
						d="M0 0h20v1H0z"
						fill={fill}
						fillOpacity="1"
						fillRule="evenodd"
					/>
				</pattern>
			);

		case "chevrons":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="40"
					height="12"
					patternTransform={PATTERN_SCALE}
				>
					<path
						d="M0 6.172L6.172 0h5.656L0 11.828V6.172zm40 5.656L28.172 0h5.656L40 6.172v5.656zM6.172 12l12-12h3.656l12 12h-5.656L20 3.828 11.828 12H6.172zm12 0L20 10.172 21.828 12h-3.656z"
						fill={fill}
						fillOpacity="1"
						fillRule="evenodd"
					/>
				</pattern>
			);

		case "diagonal_stripes":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="30"
					height="30"
					patternTransform={PATTERN_SCALE}
				>
					<g fill={fill} fillOpacity="1" fillRule="evenodd">
						<path d="M0 30L30 0H15L0 15M30 30V15L15 30" />
					</g>
				</pattern>
			);

		case "hexagons":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="28"
					height="49"
					patternTransform={PATTERN_SCALE}
				>
					<g fillRule="evenodd">
						<g fill={fill} fillOpacity="1" fillRule="nonzero">
							<path d="M13.99 9.25l13 7.5v15l-13 7.5L1 31.75v-15l12.99-7.5zM3 17.9v12.7l10.99 6.34 11-6.35V17.9l-11-6.34L3 17.9zM0 15l12.98-7.5V0h-2v6.35L0 12.69v2.3zm0 18.5L12.98 41v8h-2v-6.85L0 35.81v-2.3zM15 0v7.5L27.99 15H28v-2.31h-.01L17 6.35V0h-2zm0 49v-8l12.99-7.5H28v2.31h-.01L17 42.15V49h-2z" />
						</g>
					</g>
				</pattern>
			);

		case "waves":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="100"
					height="18"
					patternTransform={PATTERN_SCALE}
				>
					<path
						fill={fill}
						fillOpacity="1"
						d="M61.82 18c3.47-1.45 6.86-3.78 11.3-7.34C78 6.76 80.34 5.1 83.87 3.42 88.56 1.16 93.75 0 100 0v6.16C98.76 6.05 97.43 6 96 6c-9.59 0-14.23 2.23-23.13 9.34-1.28 1.03-2.39 1.9-3.4 2.66h-7.65zm-23.64 0H22.52c-1-.76-2.1-1.63-3.4-2.66C11.57 9.3 7.08 6.78 0 6.16V0c6.25 0 11.44 1.16 16.14 3.42 3.53 1.7 5.87 3.35 10.73 7.24 4.45 3.56 7.84 5.9 11.31 7.34zM61.82 0h7.66a39.57 39.57 0 0 1-7.34 4.58C57.44 6.84 52.25 8 46 8S34.56 6.84 29.86 4.58A39.57 39.57 0 0 1 22.52 0h15.66C41.65 1.44 45.21 2 50 2c4.8 0 8.35-.56 11.82-2z"
					/>
				</pattern>
			);

		case "rails":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="20"
					height="10"
					patternTransform={PATTERN_SCALE}
				>
					<path
						d="M16 6H6v4H4V6H2V4h2V0h2v4h10V0h2v4h2v2h-2v4h-2V6z"
						fill={fill}
						fillOpacity="1"
						fillRule="evenodd"
					/>
				</pattern>
			);

		case "circles":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="56"
					height="28"
					patternTransform={PATTERN_SCALE}
				>
					<path
						fill={fill}
						fillOpacity="1"
						d="M56 26v2h-7.75c2.3-1.27 4.94-2 7.75-2zm-26 2a2 2 0 1 0-4 0h-4.09A25.98 25.98 0 0 0 0 16v-2c.67 0 1.34.02 2 .07V14a2 2 0 0 0-2-2v-2a4 4 0 0 1 3.98 3.6 28.09 28.09 0 0 1 2.8-3.86A8 8 0 0 0 0 6V4a9.99 9.99 0 0 1 8.17 4.23c.94-.95 1.96-1.83 3.03-2.63A13.98 13.98 0 0 0 0 0h7.75c2 1.1 3.73 2.63 5.1 4.45 1.12-.72 2.3-1.37 3.53-1.93A20.1 20.1 0 0 0 14.28 0h2.7c.45.56.88 1.14 1.29 1.74 1.3-.48 2.63-.87 4-1.15-.11-.2-.23-.4-.36-.59H26v.07a28.4 28.4 0 0 1 4 0V0h4.09l-.37.59c1.38.28 2.72.67 4.01 1.15.4-.6.84-1.18 1.3-1.74h2.69a20.1 20.1 0 0 0-2.1 2.52c1.23.56 2.41 1.2 3.54 1.93A16.08 16.08 0 0 1 48.25 0H56c-4.58 0-8.65 2.2-11.2 5.6 1.07.8 2.09 1.68 3.03 2.63A9.99 9.99 0 0 1 56 4v2a8 8 0 0 0-6.77 3.74c1.03 1.2 1.97 2.5 2.79 3.86A4 4 0 0 1 56 10v2a2 2 0 0 0-2 2.07 28.4 28.4 0 0 1 2-.07v2c-9.2 0-17.3 4.78-21.91 12H30zM7.75 28H0v-2c2.81 0 5.46.73 7.75 2zM56 20v2c-5.6 0-10.65 2.3-14.28 6h-2.7c4.04-4.89 10.15-8 16.98-8zm-39.03 8h-2.69C10.65 24.3 5.6 22 0 22v-2c6.83 0 12.94 3.11 16.97 8zm15.01-.4a28.09 28.09 0 0 1 2.8-3.86 8 8 0 0 0-13.55 0c1.03 1.2 1.97 2.5 2.79 3.86a4 4 0 0 1 7.96 0zm14.29-11.86c1.3-.48 2.63-.87 4-1.15a25.99 25.99 0 0 0-44.55 0c1.38.28 2.72.67 4.01 1.15a21.98 21.98 0 0 1 36.54 0zm-5.43 2.71c1.13-.72 2.3-1.37 3.54-1.93a19.98 19.98 0 0 0-32.76 0c1.23.56 2.41 1.2 3.54 1.93a15.98 15.98 0 0 1 25.68 0zm-4.67 3.78c.94-.95 1.96-1.83 3.03-2.63a13.98 13.98 0 0 0-22.4 0c1.07.8 2.09 1.68 3.03 2.63a9.99 9.99 0 0 1 16.34 0z"
					/>
				</pattern>
			);

		case "nested_squares":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="70"
					height="70"
					patternTransform={PATTERN_SCALE}
				>
					<g fill={fill} fillOpacity="1" fillRule="evenodd">
						<path d="M0 0h35v35H0V0zm5 5h25v25H5V5zm5 5h15v15H10V10zm5 5h5v5h-5v-5zM40 5h25v25H40V5zm5 5h15v15H45V10zm5 5h5v5h-5v-5zM70 35H35v35h35V35zm-5 5H40v25h25V40zm-5 5H45v15h15V45zm-5 5h-5v5h5v-5zM30 40H5v25h25V40zm-5 5H10v15h15V45zm-5 5h-5v5h5v-5z" />
					</g>
				</pattern>
			);

		case "diagonal":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="6"
					height="6"
					patternTransform="rotate(45)"
				>
					<rect width="6" height="6" fill={fill} />
					<line
						x1="0"
						y1="0"
						x2="0"
						y2="6"
						stroke="rgba(0,0,0,1)"
						strokeWidth="4.5"
					/>
				</pattern>
			);

		case "dots-small":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="9"
					height="9"
					patternTransform={PATTERN_SCALE}
				>
					<circle cx="3" cy="3" r="3" fill={fill} fillOpacity="1" />
				</pattern>
			);

		case "dots-big":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="15"
					height="15"
					patternTransform={PATTERN_SCALE}
				>
					<circle cx="10" cy="10" r="10" fill={fill} fillOpacity="1" />
				</pattern>
			);

		case "dots":
			return (
				<pattern
					key={id}
					id={id}
					patternUnits="userSpaceOnUse"
					width="25"
					height="25"
					patternTransform={PATTERN_SCALE}
				>
					<circle cx="10" cy="10" r="10" fill={fill} fillOpacity="1" />
				</pattern>
			);

		default:
			return null;
	}
});

// Renders all pattern defs for a bars array — use inside <defs>
export const BarPatternDefs = memo(function BarPatternDefs({
	bars,
	idPrefix = "",
}) {
	return (
		<>
			{bars.map((bar) => {
				if (!bar.patternId || !bar.pattern) return null;
				const id = idPrefix ? `${idPrefix}_${bar.patternId}` : bar.patternId;
				return (
					<PatternDef key={id} id={id} pattern={bar.pattern} fill={bar.fill} />
				);
			})}
		</>
	);
});
