	...
	const warningDetails = (
		<div className="flex gap-2 flex-col text-xs">
			<div className="bg-base-300 p-4 rounded-lg">
				...
				Package names that do not exist here will not display any capacity.
				Please add them to the package group to view their capacity.
			</div>
			<div className="bg-base-300 p-4 rounded-lg">
				<BsInfoCircle className="inline mb-1 mr-2" />
				...
				should also appear here, along with their corresponding canonical names
				in the package_name columns from the database.
			</div>
			<div className="bg-base-300 p-4 rounded-lg">
				<BsInfoCircle className="inline mb-1 mr-2" />
				Package names are case insensitive. That said, be sure to include all
				possible variants of the name in the group name.
				<p className="mt-2">
					For example,
					<span className="font-semibold"> 150 mils</span>,
					<span className="font-semibold"> 150Mils </span>, and
					<span className="font-semibold"> 150_MILS </span> are considered
					different names. To group them together, both should be included in
		...
		<>
			...
			<div className="flex justify-between bg-base-200 z-10 sticky -top-8 py-4">
				<Tabs
					...
					tabClassName={""}
				/>
				<Link href={route("package.group.create")} className="btn btn-primary">
				...
				<tbody>
					{serverPackageGroup?.map((packageGroup) => (
						<tr key={packageGroup.id}>
							...
							<td className="uppercase">{packageGroup.factory}</td>
							<td>{packageGroup?.group_name || "-"}</td>
							<td>
								<div className="flex flex-col gap-2">
									{packageGroup?.packages.map((packageName) => (
										<div key={packageName.id} className="m-1">
