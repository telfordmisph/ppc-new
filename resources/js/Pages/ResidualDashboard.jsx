...
const ResidualDashboard = () => {
	...
	const {
		data: overallResidualData,
		isLoading: isOverallResidualLoading,
		errorMessage: overallResidualErrorMessage,
		fetch: overallResidualFetch,
		abort: overallResidualAbort,
	} = useFetch(route("api.wip.residual"), {
		params: {
			dateRange: dateRange || "",
		},
	});
	...
	const {
		data: residualSummaryData,
		isLoading: isResidualSummaryLoading,
		errorMessage: residualSummaryErrorMessage,
		fetch: residualSummaryFetch,
	} = useFetch(route("api.wip.residualSummary"), {
		auto: false,
	});

	const verb = isOverallResidualLoading ? "Loading" : "Showing";
	...
	return (
		<>
			...
			<div className="flex items-center justify-between mb-4">
				<h1 className="w-3/12 text-base font-bold">Residual Dashboard</h1>
				<h1 className="text-sm text-right sm:text-md">
					{!isOverallResidualLoading ? message : "Empty"}
				</h1>
			</div>

			<div className="md:flex">
				<div className="w-full md:w-4/12">
					<div className="flex justify-between flex-col w-full h-full mb-4 md:mb-0 md:pr-4">
						<div>
							...
							<div className="flex mt-4 flex-col gap-4">
								<FloatingLabelInput
									...
									onChange={(e) => {
										...
										handleDateChange(val ? new Date(val) : null, "start");
									}}
								/>
								<FloatingLabelInput
									...
									onChange={(e) => {
										const val = e.target.value;
										handleDateChange(val ? new Date(val) : null, "end");
									}}
									min={
										tempStartDate instanceof Date && !isNaN(tempStartDate)
						...
						<div className="flex w-full gap-2 mt-4 lg:flex-row">
							<button
								type="button"
							...
							<CancellableActionButton
								...
								loading={isOverallResidualLoading}
				...
				<div className="p-4 rounded-lg shadow-lg md:w-9/12 bg-base-200">
					<div className="overflow-x-auto text-center md:text-left">
						{isOverallResidualLoading ? (
							<ResidualTableSkeleton message={message} />
						) : overallResidualErrorMessage ? (
							<div className="text-red-500">
								Error: {overallResidualErrorMessage.message}
							</div>
						) : (
							<>
								<ResidualTable
								...
								<div className="flex items-center w-full px-4 space-x-2 text-sm opacity-50">
									...
									<button
										type="button"
										className="h-6 pointer-events-none btn btn-sm"
									>
										<FaChartBar />
									</button>
									<span>.</span>
			...
			<h1 className="mt-10 text-base text-center divider">
				{selectedChartStatus} Residual Summary
			</h1>
...
const ResidualTable = memo(function ResidualTable({ data, onSummaryView }) {
	...
	const overallPl1 = rows.reduce((sum, row) => sum + (data?.[row.pl1] || 0), 0);
	const overallPl6 = rows.reduce((sum, row) => sum + (data?.[row.pl6] || 0), 0);
		...
		<table className="table w-full">
			...
			<tbody>
				...
				<tr className="hover:bg-base-100">
					<th className="text-right rounded-l-lg bg-primary/20">Overall</th>
