				...
				<>
					...
					<div className="flex justify-between">
						...
						<span className="font-bold">
							{Number(result?.created.length ?? 0).toLocaleString()}
						</span>
					</div>
					...
					<div className="flex justify-between">
						...
						<span className="font-bold">
							{Number(result?.updated.length ?? 0).toLocaleString()}
						</span>
					</div>
				</>
			),
		...
		<>
			...
			<div className="card border mt-4 bg-base-100 border-base-content/20">
				<div className="card-body">
					<h2 className="card-title">Upload {packageCapacityLabel}</h2>
					...
					<div className="card-actions justify-end">
						<Modal
							...
							onClose={() => manualCapacityUploadRef.current?.close()}
							className="max-w-lg"
						>
							<p className="py-4">
								Make sure the package name from the file matches the package
								name in the system. Typos will cause them to be treated as
								different packages.
							</p>
							...
							<div className="flex justify-end gap-2">
								...
								<button
									type="button"
									className="btn"
									onClick={() => manualCapacityUploadRef.current?.close()}
									disabled={capacityLoading}
								>
									Cancel
								</button>
